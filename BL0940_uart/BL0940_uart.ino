...
void setup() {
  ...
  // Configure MySerial2 on pins RX=D7 and TX=D6 (-1, -1 means use the default)
  MySerial2.begin(4800, SERIAL_8N1, 13, 11);
...
bool _writeRegister(uint8_t address, uint32_t data) {
  ...
  MySerial2.write(unlockTxData, sizeof(unlockTxData));
  ...
  MySerial2.write(txData, sizeof(txData));
...
bool _readRegister(uint8_t address, uint32_t *data) {
  ...
  while (MySerial2.available() != sizeof(rxData)) {
    delay(10);
    if ((millis() - startTime) > timeout)
      break;
  }
  int rxDataLength = MySerial2.readBytes(rxData, sizeof(rxData));
